...
class MovieServiceTests: XCTestCase {
    ...
    func testGetPopularMoviesWhenConnectedToInternetAndJsonIsCorrect() async {
        ...
        do {
            ...
            XCTAssertEqual(movieResponse?.totalPages, 41367, "Should contain correct totalPages value")
        } catch {
            XCTFail("Should receive correct value for the movies. Error: \(error)")
        }
    }
    ...
    func testGetPopularMoviesWhenConnectedToInternetAndJsonIsIncorrect() async {
        ...
        do {
            _ = try await sut?.getPopularMovies(1)
            
            XCTFail("Should throw decoding error")
        } catch {
            XCTAssertEqual(error as? MovieService.NetworkError, MovieService.NetworkError.decodingError, "Should throw expected error")
        }
    }
    ...
    func testgetMovieDetailsWhenConnectedToInternetAndJsonIsCorrect() async {
        ...
        do {
            ...
            XCTAssertEqual(movieDetails?.overview, "The story of J. Robert Oppenheimer\'s role in the development of the atomic bomb during World War II.", "Should return correct overview value")
            XCTAssertEqual(movieDetails?.posterPath, "/8Gxv8gSFCU0XGDykEGv7zR1n2ua.jpg", "Should return correct posterPath value")
            XCTAssertEqual(movieDetails?.title, "Oppenheimer", "Should return correct title value")
            XCTAssertEqual(movieDetails?.homepage, "http://www.oppenheimermovie.com", "should return expected homepage value")
            XCTAssertEqual(movieDetails?.id, 872585, "Should return correct id value")
            XCTAssertEqual(movieDetails?.voteAverage, 8.23, "Should return correct voteAverage value")
        } catch {
            XCTFail("Should receive correct value for the movies. Error: \(error)")
        }
    }
